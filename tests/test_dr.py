...
def distance_from_home(
    drone_obj: drone_controller.Drone, home_position: Position
) -> float:
...
def wait_until_drone_gets_home(
    ...
    current_distance = distance_from_home(drone_obj, home_position)
...
@pytest.mark.system
def test_simulation_not_crashing_on_startup(simulation: subprocess.Popen):
    utils.wait(10, absolute=True)
    assert simulation.poll() is None, "Simulation crashed"

@pytest.mark.system
def test_drone_takeoff(flying_drone: drone_controller.Drone):
    ...
    assert (
        ...
        <= config.FLIGHT_ALTITUDE_METERS + 2
    ), "The drone failed to takeoff, its height is wrong."
...
def test_dr_trigger_gps_failure_then_rc_failure():
    """Should trigger DR"""
    assert False

@pytest.mark.system
@pytest.mark.skip(reason="WIP")
def test_dr_trigger_rc_failure_gps_healthy():
    """Should trigger RTL"""
    assert False

@pytest.mark.system
@pytest.mark.skip(reason="WIP")
def test_dr_trigger_rc_healthy_gps_failure():
    """Should trigger DR"""
    assert False

@pytest.mark.system
@pytest.mark.skip(reason="WIP")
def test_dr_trigger_gps_failure_and_rc_failure_imidiatly_after():
    """Should change to mode LAND and then cancel it and trigger DR"""
    assert False

@pytest.mark.system
@pytest.mark.skip(reason="WIP")
def test_dr_trigger_rc_failure_and_gps_failure_imidiatly_after():
