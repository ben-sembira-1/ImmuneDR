...
run_tests_in_docker:
	mkdir -p /tmp/immune-dr-test-docker/
ifdef MAVPROXY
	mavproxy.py --master tcp:localhost:${EXTERNAL_GCS_PORT} --non-interactive --map --console > /tmp/mavproxy.output 2> /tmp/mavproxy.err &
endif
	docker run -it --rm -e MAVPROXY_PORT=${MAVPROXY_PORT} -v /tmp/immune-dr-test-docker:/tmp -v $(shell pwd):/home/pilot/app ${MAVPROXY_PORT_MAPPING} sitl python3 -m pytest . ${PYTEST_ARGS}
